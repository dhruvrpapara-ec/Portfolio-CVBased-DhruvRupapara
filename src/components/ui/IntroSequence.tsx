...
export function IntroSequence({ onComplete }: IntroSequenceProps) {
  ...
  useEffect(() => {
    ...
    const sequence = async () => {
      await wait(1000); if (!isMounted) return; setPhase(1);
      await wait(2000); if (!isMounted) return; setPhase(2);
      await wait(1500); if (!isMounted) return; setPhase(3);
      await wait(2000); if (!isMounted) return; setPhase(4);
      await wait(1500); if (!isMounted) return; setPhase(5);
      await wait(2500); if (!isMounted) return; setPhase(6);
      await wait(2000); if (!isMounted) return; setPhase(7);
      await wait(1500); if (!isMounted) return; onComplete();
    };
    sequence();
...
function Node({ 
  ...
  return (
    <motion.div
      ...
      className={`
        relative px-6 py-4 md:px-12 md:py-6 rounded-[1.5rem] md:rounded-[2rem] border transition-all duration-1000
      ...
      {isDrawn && phase < glowPhase && (
        <motion.line
          ...
          transition={{ 
            ...
            repeat: Infinity, 
            ease: "linear",
            delay: (Math.random() * 0.5) // slight stagger for organic feel
          }}
